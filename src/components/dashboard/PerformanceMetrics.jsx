...
export default function PerformanceMetrics() {
    ...
    return (
        <section className="min-w-0 max-w-full select-none">
            <div className="mb-4 flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div className="min-w-0">
                    <p className="break-words text-[10px] font-bold uppercase tracking-[0.18em] text-[#DFB15B] sm:tracking-[0.26em]">Performance Ledger</p>
                    <h2 className="mt-2 break-words font-serif text-2xl font-semibold tracking-wide text-white [overflow-wrap:anywhere]">Your Practice Signal</h2>
                </div>
                <div className="inline-flex max-w-full items-center gap-2 self-start rounded-full border border-white/8 bg-white/5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-[#9D96B3] sm:w-fit sm:tracking-wider">
                    <TrendingUp className="h-3.5 w-3.5 shrink-0 text-emerald-300" />
                    <span className="min-w-0 truncate">Synced from submissions</span>
                </div>
            </div>

            <motion.div
                className="grid min-w-0 w-full max-w-full grid-cols-1 gap-5 md:grid-cols-3"
            ...
            >
                {metrics.map((item) => (
                    <motion.div
                        ...
                        whileHover={{ y: -5 }}
                        className="group relative min-h-56 min-w-0 overflow-hidden rounded-3xl border border-white/6 bg-[#121017]/92 p-5 shadow-[0_18px_55px_rgba(0,0,0,0.30)]"
                    >
                        ...
                        <div className="relative z-10 flex h-full min-w-0 flex-col">
                            <div className="flex min-w-0 items-start justify-between gap-4">
                                <div className="min-w-0">
                                    <p className="break-words text-xs font-semibold text-[#9D96B3] transition-colors group-hover:text-white">{item.title}</p>
                                    <p className="mt-3 break-words font-serif text-4xl font-bold tracking-tight text-white [overflow-wrap:anywhere]">{item.value}</p>
                                </div>
                            ...
                            <p className="mt-3 min-h-10 break-words text-sm font-medium leading-5 text-[#6B667B]">{item.subtext}</p>

                            <div className="mt-auto pt-6">
                                <div className="mb-2 flex min-w-0 items-center justify-between gap-3 text-[10px] font-bold uppercase tracking-wide text-[#6B667B] sm:tracking-wider">
                                    <span className="min-w-0 truncate">{item.footerLabel}</span>
                                    <span className="shrink-0 text-white">{item.footerValue}</span>
                                </div>
