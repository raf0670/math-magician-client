...
export default function AnalyticsPortal() {
    ...
    const overviewMetrics = useMemo(() => {
        ...
        const totalPointsEarned = Number(stats?.totalPointsEarned || 0);
        ...
        <div className="w-full flex flex-col gap-8 text-left select-none">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full">
                {[
                    ...
                    { label: "Points Earned", val: overviewMetrics.totalPointsEarned.toFixed(2), icon: Zap, color: "text-rose-400" }
                ].map((item, i) => (
