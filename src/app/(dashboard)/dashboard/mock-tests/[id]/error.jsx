...
import { ArrowLeft, Brain, FileWarning, RefreshCw, ShieldCheck, TimerReset } from "lucide-react";

const CHECKS = [
  "Question paper request stalled",
  "Answer panel was not changed",
  "Timer can be restarted safely",
];
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0A090F] px-4 py-8 text-white sm:px-6">
      <div className="pointer-events-none absolute inset-0">
        ...
        />
        <motion.div
          className="absolute left-1/2 top-1/2 h-96 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-red-300/12"
      ...
      <motion.section
        className="relative z-10 grid w-full max-w-5xl overflow-hidden rounded-3xl border border-red-400/15 bg-[#121017]/92 shadow-[0_32px_100px_rgba(0,0,0,0.55)] backdrop-blur lg:grid-cols-[0.9fr_1.1fr]"
      ...
      >
        <div className="relative flex min-h-80 items-center justify-center overflow-hidden border-b border-white/6 bg-black/20 p-8 lg:border-b-0 lg:border-r">
          ...
          <div className="relative flex h-44 w-44 items-center justify-center rounded-full border border-red-400/20 bg-red-500/10 shadow-[0_0_70px_rgba(239,68,68,0.18)]">
            <motion.span
              className="absolute inset-5 rounded-full border border-[#DFB15B]/20"
              animate={shouldReduceMotion ? { opacity: 0.7 } : { scale: [0.95, 1.06, 0.95], opacity: [0.45, 0.9, 0.45] }}
              transition={{ duration: 2.6, repeat: Infinity, ease: "easeInOut" }}
            />
            <FileWarning className="h-20 w-20 text-red-200" />
          </div>
        </div>
        ...
        <div className="p-6 sm:p-8 lg:p-10">
          ...
          <div className="mt-6 grid gap-3">
            {CHECKS.map((check) => (
              <div key={check} className="flex items-center gap-3 rounded-xl border border-white/6 bg-white/[0.035] px-4 py-3">
                <ShieldCheck className="h-4 w-4 shrink-0 text-[#DFB15B]" />
                <span className="text-xs font-semibold text-[#A9A3BA]">{check}</span>
              </div>
            ))}
          </div>
