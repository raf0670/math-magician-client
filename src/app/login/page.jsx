...
import { clearPendingPaymentPlan, clearPendingProgramAction, getPendingPaymentPlan, getPendingProgramAction, loginUser, saveAuthSession } from "@/lib/api";
    ...
    <div className="min-h-screen bg-[#0A090F] text-white flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#121017] p-8 shadow-2xl">
        <div className="mb-8 text-left">
          ...
          <h1 className="mt-2 font-sans text-3xl font-medium">Welcome back</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="mb-1 flex items-center justify-between gap-3">
              <label className="block text-xs font-semibold uppercase tracking-wide text-[#8E8A9F]">Password</label>
              <Link href="/forgot-password" className="text-xs font-semibold text-[#DFB15B] transition hover:brightness-110">
                Forgot password?
              </Link>
            </div>
            <input
