  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#0A090F] px-4 text-white">
      <div className="w-full max-w-md rounded-3xl border border-[#DFB15B]/20 bg-[#121017] p-8 text-center shadow-2xl">
        ...
        <p className="mt-6 text-xs font-bold uppercase tracking-[0.3em] text-[#DFB15B]">{isBooking ? "Seat Booked" : "Review Pending"}</p>
        <h1 className="mt-3 font-serif text-3xl font-medium">{isBooking ? "Your seat is reserved" : "Enrollment submitted"}</h1>
        <p className="mt-3 text-sm leading-6 text-[#8E8A9F]">
          {isBooking
            ? "Your student information has been saved. Class access is still locked until you proceed to checkout and an admin approves the payment."
            : isPartial
              ...
              : "Your transaction ID and enrollment form are waiting for admin approval. Class access will unlock after approval."}
        </p>
        {paymentId ? (
          <div className="mt-5 rounded-2xl border border-white/5 bg-[#1A1722] px-4 py-3 text-xs text-[#8E8A9F]">
            Review ID: <span className="font-semibold text-white">{paymentId}</span>
          </div>
        ) : null}
        <Link
...
function PaymentReviewLoading() {
  return (
    <FlashyLoader
      ...
      message="Your review request is being loaded."
      iconName="credit"
      skeleton="cards"
      surface="screen"
    />
